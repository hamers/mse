        ...
        for (it = particlesMap->begin(); it != particlesMap->end(); it++)
        {
            ...
            if ((parent->is_binary == 1) && (parent->level == level))
            {
                ...
                else
                {
                    ...
                    //printf("parent->sample_orbital_phases_randomly  %d %g seed %d\n",parent->sample_orbital_phase_randomly,true_anomaly);//,seed+index);
                }
                ...
                from_orbital_vectors_to_cartesian(
                    ...
                    true_anomaly,
                    r,v);
                //printf("fov %g %g %g %g %g %g\n",e_vec[0],e_vec[1],e_vec[2],r[0],r[1],v[0]);
...
int apply_external_perturbation_assuming_integrated_orbits(ParticlesMap *particlesMap)
{
    ...
    //extract_ODE_variables(particlesMap, y, 0.0);
...
void apply_external_perturbation_assuming_integrated_orbits_binary_pair(ParticlesMap *particlesMap, int binary_index, int perturber_index)
{
    ...
    /* NOTE Dec 11 2019:
     * Need to update this -- pre-processed quantities like e_vec_unit etc. are now potentially undefined with the change in apply_external_perturbation_assuming_integrated_orbits */
