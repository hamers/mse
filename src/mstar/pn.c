...
void compute_PN_twobody_Acc(struct RegularizedRegion *R, double *Vel, double *AccPN, int i, int j ){
    ...
    // spin contributions to acceleration
    double A_SS[3] = {0};
    double A_SO[3] = {0};
    double A_Q[3]  = {0};
    ...
    for(int k=0;k<3;k++){
 	A_SO[k] = G/(c2*r3) * (nv[k]*A_SO_term1 - A_SO_term2[k] + A_SO_term3[k]);
	A_SS[k] = -3.0*G/(mu * c2 * r4) * ( S1_dot_S2 * nv[k] - 5 * n_dot_S1 * n_dot_S2 * nv[k] + n_dot_S1 * S_2[k] + n_dot_S2 * S_1[k] );
	A_Q[k]  = -3.0/2 * G*m/(r4 * c2) * ( ( 1/(m1*m1) * ( (S1_2 - 5*n_dot_S1*n_dot_S1)*nv[k] + 2*n_dot_S1 * S_1[k]) ) + ( 1/(m2*m2) * ( (S2_2 - 5*n_dot_S2*n_dot_S2)*nv[k] + 2*n_dot_S2 * S_2[k]) ));
    }
